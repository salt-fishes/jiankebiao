...
### v1.5（2026-09-02）
- 🗂 **多课表管理**：班级课表 / 个人课表 / 同学的课表并存，顶栏面板快速切换（周数滑杆 + 课表卡片）
- 🆕 **新建课表重构**：可选导入文件或复制现有课表，Excel/PDF 自动命名并识别开学日与总周数
- ⏰ **每张课表独立配置**：开学日、总周数、作息时间互不影响（小组件按绑定课表计算）
- 📱 小组件按实例绑定课表（3×2 / 2×2），标题显示课表名
- 🐛 修复：开学日前后 6 天内周次误判为第 1 周（含小组件开学前仍显示课程）
- 🐛 修复：课表管理 / 小组件设置页与设置界面重叠；接入系统手势返回
